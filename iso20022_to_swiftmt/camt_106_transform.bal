...
isolated function transformCamt106ToMtn91(camtIsoRecord:Camt106Envelope envelope, string messageType)
    returns swiftmt:MTn91Message|error => let
        ...
        swiftmt:MT52A?|swiftmt:MT52B?|swiftmt:MT52D? chrgsAcctAgt = check getField52(
            ...
            envelope.Document.ChrgsPmtReq.GrpHdr.ChrgsAcctAgtAcct?.Id)
    in {
        ...
        block4: {
            MT20: {
                name: MT20_NAME,
                msgId: {
                    content: getMxToMTReference(charges?.ChrgsId.toString()),
                    number: NUMBER1
                }
            },
            ...
            MT32B: chargesRecord is camtIsoRecord:ChargesPerTransactionRecord3[] ? {
                    ...
                    Amnt: {
                        content: convertDecimalToSwiftDecimal(chargesRecord[0].TtlChrgsPerRcrd?.TtlChrgsAmt?.content),
                        number: NUMBER2
                    }
                } : {
                    name: MT32B_NAME,
                    Ccy: {content: "", number: NUMBER1},
